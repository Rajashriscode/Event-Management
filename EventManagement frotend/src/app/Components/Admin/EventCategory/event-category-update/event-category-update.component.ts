...
export class EventCategoryUpdateComponent implements OnInit {
  ...
  categoryForm: FormGroup = this._formBuilder.group({
    event_category: [
      this.category,
      [Validators.required, Validators.pattern('[a-z0-1A-Z ]+')],
    ],
  });
  constructor(
  ...
  updateCategory() {
    this._event_category_service
      .updateCategory(this.last_category, this.category)
      .subscribe();
    this._router.navigate(['/admin/event_category']);
  }
}
